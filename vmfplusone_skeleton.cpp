...
struct vmfplusone
{
  ...
  int16_t decode_src_8() {
   ...
   int16_t decode_src_11() {
    int16_t src;
      ...
      else {  // immediate operand for next 11 bits
        return (IR & 0x07ff);
      }
  }//decode_src_11
  
   int16_t decode_src_12() {
    int16_t src;
      if (IR & 0x0800) { // register operand
        return REG[(IR & 0x0eff)];
      }
      else {  // immediate operand for next 12 bits
        return (IR & 0x0eff);
      }
  }//decode_src_12
  ...
  uint16_t decode_dst() { return IR % 8; }//right 3 bits
...
static const string err_message[] = {
  ...
  "Illegal Opcode",
  "Invalid Operand",
  "Unable to access or overwrite data outside RAM"
};
...
void vmfplusone::add() {    // must decode operands from IR
  ...
  void vmfplusone::div() {   // must decode operands from IR
    int16_t src = decode_src_8();
    uint16_t dst = decode_dst();
    REG[CX] = REG[dst] % src; //sets CX to remainder
    REG[dst] = REG[dst]/src;
  }    

void vmfplusone::push() { //code to work with cx
    int16_t src = decode_src_11();
    if (REG[SP] > 12287) {throw 0;} else if (REG[SP] < 4096) {throw 1;} else {
      RAM[REG[SP]++] = src;
    }
  }//push
  ...
  void vmfplusone::load() { // load from [mar] to dst
     uint16_t dst = decode_dst();
     if (REG[MAR] < 4096 || REG[MAR] > 12287) {throw 5;} else {
       REG[dst] = RAM[REG[MAR]];
     }
  }//load
...
void print_inst(uint16_t inst)
{
  uint16_t IR = inst;
  int op = ((IR & 0xF000) >> 12);
  bool isReg = (IR & 0x0800);
  
  cout << Instruction[(IR & 0xF000) >> 12] << " "; //instruction
  //alu inst start
  if (!(isReg) && (((op > 0) && (op <= 4)) || (op == 7)))
    {cout << ((IR & 0x07F8) >> 3) << " " <<  Register[IR & 0x0007];} //first 5 ops and mov with 1 register
  
  if (isReg && (((op > 0) && (op <= 4)) || (op == 7)))
    {cout << (Register[(IR & 0x07F8) >> 3]) << " " <<  Register[IR & 0x0007];} //first 5 ops and mov with 2 registers
  //alu inst end
  //mem inst start
  if (op == 6 || op == 8) {cout <<  Register[IR & 0x0007];} //pop and load
  if (!(isReg) && (op == 5 || op == 9)) {cout << (IR & 0x07ff);}//push and store
  if (isReg && (op == 5 || op == 9)) {cout << Register[(IR & 0x07ff)];}//push and store
  //mem inst end
  //branch inst start
  if (op >= 10 && op <= 14) {cout << (IR & 0x0fff);}
